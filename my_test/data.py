...
class dataProcess(object):
	def __init__(self, out_rows, out_cols, data_path = "../deform/train/train/256", label_path = "../deform/train/label/256", test_path = "../test/256", npy_path = "../npydata", img_type = "tif"):
        # 数据处理类，初始化
	...
	def resize_train_data(self):
		...
		for imgname in imgs:
			...
			cv2.imwrite(self.data_path + "/" + "256/" + midname,imgs)
			label = cv2.imread(self.label_path + "/" + midname)
			labels = cv2.resize(label, (256, 256))
			cv2.imwrite(self.label_path + "/" + "256/" + midname,labels)
			if i % 100 == 0:
	...
	def resize_test_data(self):
		...
		for imgname in imgs:
			...
			imgs = cv2.resize(img, (256, 256))
			cv2.imwrite(self.test_path + "/" + "256/" + midname,imgs)
			if i % 100 == 0:
				print('Done: {0}/{1} images'.format(i, len(imgs)))
			i += 1
		print('resize done')
...
# 以下注释掉的部分为数据增强代码，通过他们可以将数据进行增强
	...
	#mydata.resize_train_data()
	#mydata.resize_test_data()
